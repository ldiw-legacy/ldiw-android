	...
	private Runnable getObjectNet = new Runnable(){
		...
		public void run() {
			...
			try{
				...
				pointUrl = url+AppConstants.ADDRESS_WASTE_POINT_MAXRES+max+
				AppConstants.ADDRESS_WASTE_POINTS_NEAREST+location.getLongitude()+","+location.getLatitude();
				...
				while (pointsData == null){	
					try {
						...
						pointUrl = url+AppConstants.ADDRESS_WASTE_POINT_MAXRES+max
						+AppConstants.ADDRESS_WASTE_POINTS_BBOX+(location.getLongitude()-bbox)+","+(location.getLatitude()-(2*bbox)/3)+","+(location.getLongitude()+bbox)+","+(location.getLatitude()+(2*bbox)/3);			
						InputStream is = Downloader.connect(pointUrl, null);		
						pointsData = Downloader.decodeCSV(is);
					}catch (Exception ex) {
						pointsData = null;
